...
class TTSService(ComfyBaseService):
    ...
    async def _call_comfyui_workflow(
        ...
        **params
        ...
        # Add a random seed to prevent ComfyUI execution caching
        import random
        if "seed" not in workflow_params:
            workflow_params["seed"] = random.randint(1, 999999)
        ...
        try:
            ...
            if audio_path and not audio_path.startswith(('http://', 'https://')):
                base_url = comfyui_url or self.global_config.get("comfyui_url") or "http://127.0.0.1:8188"
                base_url = base_url.rstrip('/')
                
                # If it's a direct API view format
                if "view?filename=" in audio_path:
                    audio_path = f"{base_url}/{audio_path.lstrip('/')}"
                else:
                    # If it's just a file name or relative path like 'audio/ComfyUI_00008_.mp3'
                    # ComfyUI file API: view?filename=xxx&subfolder=yyy&type=zzz
                    filename = os.path.basename(audio_path)
                    subfolder = os.path.dirname(audio_path)
                    if subfolder:
                        audio_path = f"{base_url}/view?filename={filename}&subfolder={subfolder}&type=output"
                    else:
                        audio_path = f"{base_url}/view?filename={filename}&type=output"
                logger.info(f"🔗 自动转换 ComfyUI 相对路径为局域网下载 URL: {audio_path}")

            # If no output_path provided but we got a ComfyUI HTTP URL, force download to local temporary file to avoid browser CORS/Range issues
            if not output_path and audio_path.startswith(('http://', 'https://')):
                import uuid
                unique_id = uuid.uuid4().hex
                output_path = f"output/{unique_id}.mp3"
                logger.info(f"💾 强制开启局域网文件本地化下载，自动分派本地缓存路径: {output_path}")
